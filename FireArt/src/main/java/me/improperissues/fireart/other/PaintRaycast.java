...
import me.improperissues.fireart.data.SelectedBlock;
...
public class PaintRaycast {
    ...
    public static SelectedBlock getSelectedBlock(Player player) {
        Location loc = player.getEyeLocation();
        Vector dir = player.getLocation().getDirection();
        ...
        for (int i = 0; i < 200; i ++) {
            ...
            Location newLoc = loc.clone().add(x,y,z);
            Block block = newLoc.getBlock();
            if (!block.isPassable() && block.getType().isBlock()) return new SelectedBlock(block);
        }
        return null;
    }
}
